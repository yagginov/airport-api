...
class TestAirplaneApi(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.airplanes = [
            Airplane.objects.create(name="Boeing 737-800", rows=30, seats_in_row=6, airplane_type=cls.type1),
            Airplane.objects.create(name="Boeing 737-900", rows=32, seats_in_row=6, airplane_type=cls.type1),
            Airplane.objects.create(name="Airbus A320neo", rows=28, seats_in_row=6, airplane_type=cls.type2),
        ]
    ...
    def test_create_airplane_admin(self):
        self.authenticate(self.admin)
        url = reverse("airport:aiplane-list")
        data = {"name": "Boeing 737 MAX", "rows": 33, "seats_in_row": 6, "airplane_type": self.type1.id}
    ...
    def test_create_airplane_user(self):
        self.authenticate(self.user)
        url = reverse("airport:aiplane-list")
        data = {"name": "Airbus A321", "rows": 34, "seats_in_row": 6, "airplane_type": self.type2.id}
        response = self.client.post(url, data)
    ...
    def test_create_airplane_anon(self):
        url = reverse("airport:aiplane-list")
        data = {"name": "Embraer E190", "rows": 25, "seats_in_row": 4, "airplane_type": self.type2.id}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_update_airplane_admin(self):
        ...
        obj = self.airplanes[0]
        url = reverse("airport:aiplane-detail", args=[obj.id])
        data = {"name": "Boeing 737-800 Updated", "rows": 30, "seats_in_row": 6, "airplane_type": self.type1.id}
    ...
    def test_update_airplane_user(self):
        ...
        obj = self.airplanes[1]
        url = reverse("airport:aiplane-detail", args=[obj.id])
        data = {"name": "Boeing 737-900 Updated", "rows": 32, "seats_in_row": 6, "airplane_type": self.type1.id}
        response = self.client.put(url, data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        obj.refresh_from_db()
    ...
    def test_update_airplane_anon(self):
        obj = self.airplanes[2]
        url = reverse("airport:aiplane-detail", args=[obj.id])
        data = {"name": "Airbus A320neo Updated", "rows": 28, "seats_in_row": 6, "airplane_type": self.type2.id}
        response = self.client.put(url, data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        obj.refresh_from_db()
    ...
    def test_ordering_airplane(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        sorted_by_capacity = sorted(self.airplanes, key=lambda x: x.rows * x.seats_in_row)
        response_names = [t["name"] for t in response.data]
